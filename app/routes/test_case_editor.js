...
exports.postGherkin = function(req, res) {
    ...
    db.sequelize.query(`SELECT * FROM TestCase;`).then(gherkinData => {  // TestCase table has: TestCaseId, HashValue, TestCaseDescription, Live, Gherkin
        db.sequelize.query('SELECT * FROM Template;').then(whereUsed => { //Template has: Id, TestCaseId ->     f1 - 1,2,3,4,
            ...
            // writing to the database the new or updated Gherkin
            if(req.body[0].theID == ""){
                db.sequelize.query("INSERT INTO TestCase (HashValue,TestCaseDescription,Live,Gherkin) VALUES ('" + gherkinData.length +"-new', '" + req.body[0].theScenario + "' , 1 , '" + CompleteGherkin + "');" ).catch(function(err) {
                    console.log('error: ' + err);
                    return err;
                })
                console.log("this is a new ID");
            } else {
                db.sequelize.query("UPDATE TestCase SET HashValue = '" + req.body[0].theID + "-update', TestCaseDescription = '" + req.body[0].theScenario + "', Live = 1, Gherkin = '" +CompleteGherkin +"' WHERE TestCaseId = '" + req.body[0].theID + "'; "  ).catch(function(err) {
                    console.log('error: ' + err);
                    return err;
                })
                console.log("there was an ID already");
            }
            ...
            // from the textareas pulled off the page (selections to update to the database)
                // console.log(req.body[0].theID + "<-----------------------the ID");
                // console.log(req.body[0].newPages + "<------------------- add to these pages");
                // console.log(req.body[0].removals + "<------------------- remove from these pages");
                // console.log(req.body[0].removals[0] + "<-----------------remove from this one first");
                // console.log(whereUsed[3].TestCaseId + "<-----------------whereUsed[3]");
